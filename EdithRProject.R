...
pdf("figures/weightdistribution.pdf")
...
# Also saves the figure as a pdf
pdf("figures/weightovertime.pdf")
ggplot(data=Avg, aes(x=year, y= x)) + geom_point(shape=16,size=5,color="#9900cc") + geom_smooth(method=lm, colour="#9900cc") + ylab("Mean Weight (g)") + xlab("Year") + theme_bw() + ggtitle("Mean Weight of D. merriami over Time") + scale_x_continuous(breaks=seq(1976,2003,2))
dev.off()
